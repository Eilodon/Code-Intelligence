...
impl CodeIntelligenceServer {
    ...
    pub(crate) fn edit_lines(&self, #[tool(aggr)] p: EditLinesParams) -> String {
        self.timed_tool("edit_lines", || {
            let hunks: Vec<ci_core::edit::HunkRequest> = p
                .edits
                .into_iter()
                .map(|h| ci_core::edit::HunkRequest {
    ...
    pub(crate) fn edit_symbol(&self, #[tool(aggr)] p: EditSymbolParams) -> String {
        self.timed_tool("edit_symbol", || {
            ...
            let hunk = ci_core::edit::HunkRequest {
    ...
    /// hunks in-memory (all-or-nothing, see `ci_core::edit::apply_hunks`) →
    ...
    fn edit_lines_impl(
        &self,
        path: &str,
        hunks: Vec<ci_core::edit::HunkRequest>,
        confirm: bool,
    ) -> String {
        ...
        let outcome = match ci_core::edit::apply_hunks(&original, &hunks) {
            Ok(o) => o,
        ...
        let ext = std::path::Path::new(path)
            ...
            .and_then(|e| e.to_str())
            .unwrap_or("");
        let parse_status = match ci_core::edit::validate_syntax(&new_content, ext) {
            Some(true) => "clean",
            Some(false) => {
                return error_json(
                    "PARSE_ERROR",
        ...
        if let Err(e) = ci_core::edit::atomic_write(&full_path, &new_content) {
            drop(_guard);
            return error_json(
                "WRITE_FAILED",
        ...
        match ci_core::indexer::pipeline::reindex_changed(&mut write_conn, &self.project_root) {
            Ok(summary) if !summary.is_noop() => {
                if let Some(model) = self.embedder() {
                    if let Err(e) = ci_core::embedding::embed_pending(&write_conn, model.as_ref()) {
                        tracing::error!("edit_lines: incremental embedding failed: {e}");
                    }
                    if let Err(e) =
                        ci_core::embedding::embed_pending_chunks(&write_conn, model.as_ref())
                    {
...
impl From<&ci_core::edit::HunkResult> for EditHunkResultOutput {
    fn from(r: &ci_core::edit::HunkResult) -> Self {
        let applied = r.status == ci_core::edit::HunkStatus::Applied;
        Self {
            start_line: r.start_line as i64,
            end_line: r.end_line as i64,
            status: match r.status {
                ci_core::edit::HunkStatus::Applied => "applied",
                ci_core::edit::HunkStatus::Preview => "preview",
                ci_core::edit::HunkStatus::Conflict => "conflict",
            }
            .to_string(),
