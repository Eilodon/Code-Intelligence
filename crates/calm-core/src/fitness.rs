...
/// Called from `calm doctor` — not on the `fitness-check` hot path, since
...
mod tests {
    ...
    fn test_load_boundary_rules_from_same_toml_file() {
        ...
        write!(
            ...
            "[thresholds]\nmax_hub_count = 5\n\n\
             [[boundaries]]\n\
             from = \"crates/calm-core/\"\n\
             to = \"crates/calm-server/\"\n\
        ...
        assert_eq!(rules[0].from, "crates/calm-core/");
        assert_eq!(rules[0].to, "crates/calm-server/");
    ...
    fn test_boundary_violations_fail_gates_fitness_check() {
        ...
        conn.execute(
            "INSERT INTO import_edges (from_path, to_path, module_name) \
             VALUES ('crates/calm-core/src/x.rs', 'crates/calm-server/src/y.rs', 'y')",
            [],
        )
        .unwrap();
